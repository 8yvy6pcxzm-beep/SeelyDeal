...
- ADIM 4 — TEK KONSOLİDE ONAY (ÇOK ÖNEMLİ): Tüm bilgiler toplandıktan (veya mümkün olduğunca toplandıktan) SONRA, bunları TEK BİR mesajda özetleyip açıkça onay iste — örn. "Bu bilgileri varsayılan olarak şirket profiline ekliyorum, onaylıyor musun?" dedikten sonra kısa bir liste: "Şirket adı: ..., E-posta: ..., Slogan: ..., Marka rengi: ..., Hizmetler/fiyatlandırma: ...". Kullanıcı "evet/onaylıyorum/tamam" gibi net bir onay verene kadar \`\`\`brand\`\`\` veya \`\`\`instruction\`\`\` bloklarını EKLEME — bu bilgiler kullanıcı açıkça onaylamadan KAYDEDİLMEZ. Kullanıcı onaylarsa, o turun cevabında (kısa bir teşekkür/teyit cümlesiyle) \`\`\`brand\`\`\` bloğuna toplanan TÜM alanları doldur: {"name": "...", "email": "...", "tagline": "...", "setLogo": ..., "primaryColor": ...} (sadece gerçekten bilgi varsa doldur, yoksa alanı hiç ekleme) VE hizmet/fiyatlandırma özetini AYRICA \`\`\`instruction\`\`\` bloğuna yaz ki kalıcı AI talimatı olarak saklansın (örn. {"text": "Hizmetlerimiz: X, Y. Fiyatlandırma: Z."}) — brand bloğunda "servicesSummary" gibi bir alan YOK, hizmet/fiyatlandırma bilgisi SADECE instruction bloğu üzerinden kaydedilir. Kullanıcı bazı alanları reddederse/düzeltirse, sadece düzeltilenle güncelleyip tekrar TEK bir onay iste.
...
${pricingBlock}${websiteContext}${prefillBlock}`;
  ...
  let brand: { setLogo?: boolean; primaryColor?: string | null; name?: string; tagline?: string; email?: string } | null = null;
  if (brandMatch) {
    try {
